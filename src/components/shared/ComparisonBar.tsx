      ...
      <div className="max-w-7xl mx-auto px-3 md:px-4 py-3 md:py-4">
        ...
        <div className="flex md:hidden items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <ArrowLeftRight className="w-5 h-5 text-site-accent flex-shrink-0" />
            ...
            {comparisonCars.map((car) => (
              <div key={car._id} className="relative flex-shrink-0">
                <Image
                  ...
                  className="w-12 h-12 rounded-lg object-cover border-2 border-site-accent/30"
                  height={100}
                  width={100}
                />
                <button
        ...
        <div className="hidden md:flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <ArrowLeftRight className="w-5 h-5 text-site-ccent" />
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              ...
              className={`px-6 py-2 rounded-lg font-semibold text-sm transition-all ${
                comparisonCars.length >= 2
                  ? "bg-gradient-to-r from-site-accent to-slate-teal text-white shadow-lg hover:shadow-xl"
                  : "bg-soft-grey/30 text-grey cursor-not-allowed"
              }`}
              onClick={(e) => {
                if (comparisonCars.length < 2) e.preventDefault();
