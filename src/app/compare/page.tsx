...
export default function ComparePage() {
  ...
  const getValue = (car: CarTypes, path: string): string => {
    ...
    <div className="min-h-screen bg-off-white">
      ...
      <div className="pt-32 pb-24">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          ...
          {/* TABLE */}
          <div className="bg-white rounded-2xl shadow-xl border border-soft-grey/20">
            <div className="w-full overflow-x-auto scrollbar-thin scrollbar-thumb-soft-grey/40 scrollbar-track-transparent">
              <table className="w-full min-w-[700px] md:min-w-0 table-fixed">
                <colgroup>
                  <col className="w-[120px] md:w-[220px]" />
                  {comparisonCars.map(() => (
                    <col key={Math.random()} />
                  ))}
                </colgroup>
                ...
                <thead>
                  <tr className="bg-gradient-to-r from-accent/5 to-slate-teal/5">
                    <th className="sticky left-0 bg-white z-10 px-2 md:px-6 py-2 md:py-4 text-left font-bold border-r border-soft-grey/20 text-xs md:text-base">
                      Feature
                    </th>
                    {comparisonCars.map((car) => (
                      <th
                        key={car._id}
                        className="px-3 md:px-6 py-3 md:py-4 min-w-[200px] md:min-w-[280px]"
                      >
                        <div className="relative">
                          <button
                            onClick={() => removeFromComparison(car._id)}
                            className="absolute -top-2 -right-2 p-1 bg-white rounded-full shadow-lg text-grey hover:text-danger transition-colors z-10"
                          >
                          ...
                          <div className="flex items-center justify-center gap-1 md:gap-2 mt-1 md:mt-2">
                            <VerifiedBadge className="w-3 h-3 md:w-4 md:h-4" />
                ...
                <tbody>
                  {comparisonFields.map((field, idx) => (
                    ...
                    >
                      <td className="sticky left-0 bg-white z-10 px-2 md:px-6 py-2 md:py-4 font-semibold border-r border-soft-grey/20 text-[11px] md:text-base">
                        {field.label}
                      </td>
                      {comparisonCars.map((car) => (
                        <td
                          key={car._id}
                          className="px-3 md:px-6 py-3 md:py-4 text-center text-grey text-xs md:text-base"
                        >
                  ...
                  <tr className="bg-white">
                    ...
                    {comparisonCars.map((car) => (
                      <td
                        key={car._id}
                        className="px-3 md:px-6 py-3 md:py-4 space-y-2"
                      >
                        <a
                          href={`tel:${car.vendor?.vendorDetails?.contact?.mobileNum}`}
                          className="flex items-center justify-center gap-1 bg-gradient-to-br from-white to-off-white border-2 border-soft-grey/50 text-site-grey hover:border-site-accent hover:text-site-accent px-3 md:px-5 py-2 rounded-xl font-semibold text-xs md:text-base shadow-md"
                        >
                          <Phone className="w-3 h-3 md:w-4 md:h-4" /> Call
                        </a>
                        ...
                        <a
                          ...
                          className="flex items-center justify-center gap-1 bg-gradient-to-br from-white to-off-white border-2 border-site-accent/60 text-site-accent hover:border-site-accent hover:text-white hover:bg-gradient-to-r hover:from-site-accent hover:to-site-accent/90 px-3 md:px-5 py-2 rounded-xl font-semibold text-xs md:text-base shadow-md"
                        >
                        ...
                        <Link
                          href={`/car/${car._id}`}
                          className="block w-full text-center bg-gradient-to-r from-site-accent to-slate-teal text-white px-3 md:px-5 py-2 rounded-xl font-semibold shadow-lg hover:shadow-xl text-xs md:text-base"
                        >
                          Rent Now
                        </Link>
                      </td>
